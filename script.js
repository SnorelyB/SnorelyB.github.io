...
function update() {
  ...
  if (detectCollision(ball, player1)) {
    if (ball.x <= player1.x + player1.width) {
      ...
      ball.velocityX = getRandomInt(-1, -3)
      ball.velocityX *= -1; //flip x direction
      ball.velocityY = getRandomInt(2, 5)
    }
  }
  else if (detectCollision(ball, player2)) {
    if (ball.x + ballWidth >= player2.x) {
      //right side of ball touches left side of player2
      ball.velocityX = getRandomInt(0, 3)
      ball.velocityX *= -1; //flip x direction
      ball.velocityY = getRandomInt(2, 5)
    }
  }
  ...
  for (let i = 10; i < board.height; i += 25) {
    ...
    // (x position = half of boardWidth -10)l, i = y position, width = 5, height = 5
...
function detectCollision(a, b) {
  return a.x < b.x + b.width && //a's top left corner doesn't reack b's top right corner
